        ...
        <div className="container mx-auto">
          <div className="lg:flex gap-x-7 lg:bg-[#F0F0F0] lg:overflow-hidden rounded-2xl p-1 pt-6">
            ...
            <div className="text-black lg:px-0 px-5 lg:-translate-y-0 -translate-y-10">
              ...
              <p className="text-sm mt-2">{data.description_text}</p>
              <div className="text-xs font-normal text-primaryColor mt-2">
                <span>{data.work_date}</span>
              </div>
              <div>
                <button className="lg:mt-1 mt-2 flex w-[70px] bg-primaryColor h-[18px] items-center justify-center text-xs text-white rounded">
                  {data.adverts_count} Elan
                </button>
              </div>
            </div>
          ...
          <NavLinkProduct url1={`/whole-sales/${params.id}`} url2={`/whole-sales/${params.id}/info`} />
          {children}
        </div>
      </section>
    </main>
